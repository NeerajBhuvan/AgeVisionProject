...
class pSp(nn.Module):
	...
	def load_weights(self):
		if self.opts.checkpoint_path is not None:
			...
			if getattr(self.opts, 'start_from_encoded_w_plus', False):
			...
			# Training-from-scratch path: requires pretrained backbone weights
			ir_se50_path = model_paths.get('ir_se50', '')
			if not ir_se50_path or not os.path.isfile(ir_se50_path):
				raise FileNotFoundError(
					f"ir_se50 weights not found at '{ir_se50_path}'. "
					"Download model_ir_se50.pth to checkpoints/ or provide a checkpoint_path."
				)
			encoder_ckpt = torch.load(ir_se50_path, map_location='cpu', weights_only=False)
			...
			stylegan_path = getattr(self.opts, 'stylegan_weights', '')
			if not stylegan_path or not os.path.isfile(stylegan_path):
				raise FileNotFoundError(
					f"StyleGAN weights not found at '{stylegan_path}'. "
					"Download stylegan2-ffhq-config-f.pt to checkpoints/ or provide a checkpoint_path."
				)
			print(f'Loading decoder weights from pretrained path: {stylegan_path}')
			ckpt = torch.load(stylegan_path, map_location='cpu', weights_only=False)
