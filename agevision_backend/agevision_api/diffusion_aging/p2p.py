"""
Prompt-to-Prompt attention control for FADING.
Adapted from MunchkinChen/FADING (BMVC 2023).

Provides cross/self-attention replacement, refinement, and reweighting
controllers used during the diffusion sampling loop.
"""
...
class LocalBlend:
    ...
    def __call__(self, x_t, attention_store):
        ...
        if self.counter > self.start_blend:
            ...
            if self.substruct_layers is not None:
                maps_sub = ~self.get_mask(x_t, maps, self.substruct_layers,
                                          False)
                mask = mask * maps_sub
            mask = mask.float()
    ...
    def __init__(self, prompts, words, tokenizer, substruct_words=None,
                 start_blend=0.2, th=(.3, .3)):
        device = next(iter(torch.cuda.device_count() and [torch.device('cuda')]
                          or [torch.device('cpu')]))
        ...
        if is_cross or (self.num_self_replace[0] <= self.cur_step <
            ...
            if is_cross:
                ...
                attn_repalce_new = (
                    ...
                    alpha_words + (1 - alpha_words) * attn_repalce
                )
                attn[1:] = attn_repalce_new
            else:
                attn[1:] = self.replace_self_attention(attn_base,
                                                        attn_repalce,
                                                        place_in_unet)
...
class AttentionRefine(AttentionControlEdit):
    def replace_cross_attention(self, attn_base, att_replace):
        ...
        attn_replace = (attn_base_replace * self.alphas +
...
def get_equalizer(text, word_select, values, tokenizer):
...
def make_controller(prompts, is_replace_controller, cross_replace_steps,
                    self_replace_steps, tokenizer, blend_words=None,
                    equilizer_params=None):
    """Build the appropriate attention controller for age editing."""
    ...
    if is_replace_controller:
        controller = AttentionReplace(
            ...
            tokenizer=tokenizer, local_blend=lb
        )
    else:
    ...
    if equilizer_params is not None:
        eq = get_equalizer(prompts[1], equilizer_params["words"],
                           equilizer_params["values"], tokenizer=tokenizer)
        controller = AttentionReweight(
            prompts, NUM_DDIM_STEPS,
            cross_replace_steps=cross_replace_steps,
            self_replace_steps=self_replace_steps,
            tokenizer=tokenizer, equalizer=eq, local_blend=lb,
...
def p2p_text2image(model, prompt, controller, num_inference_steps=NUM_DDIM_STEPS,
                   ...
                   return_type='image', height=512, width=512):
    """Run prompt-to-prompt controlled generation."""
    ...
    for i, t in enumerate(tqdm(model.scheduler.timesteps[-start_time:],
        ...
        latents = ptp_utils.diffusion_step(model, controller, latents,
