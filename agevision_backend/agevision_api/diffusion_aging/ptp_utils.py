...
#
# Prompt-to-Prompt attention utilities for FADING.
...
def diffusion_step(model, controller, latents, context, t,
    ...
    if low_resource:
        noise_pred_uncond = model.unet(latents, t,
                                       encoder_hidden_states=context[0])["sample"]
        noise_prediction_text = model.unet(latents, t,
                                           encoder_hidden_states=context[1])["sample"]
    else:
...
def init_latent(latent, model, height, width, generator, batch_size):
    ...
    latents = latent.expand(batch_size, model.unet.in_channels,
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        ...
        def forward(hidden_states, encoder_hidden_states=None, attention_mask=None,
                    **kwargs):
            # Support both old (x, context) and new (hidden_states, encoder_hidden_states)
            x = hidden_states
    ...
    def register_recr(net_, count, place_in_unet):
        # Support both old "CrossAttention" and new "Attention" class names
        cls_name = net_.__class__.__name__
        if cls_name == 'CrossAttention' or (cls_name == 'Attention' and hasattr(net_, 'to_q')):
...
def get_time_words_attention_alpha(prompts, num_steps,
    ...
    alpha_time_words = alpha_time_words.reshape(num_steps + 1, len(prompts) - 1,
                                                1, 1, max_num_words)
