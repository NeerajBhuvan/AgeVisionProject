"""
Null-Text Inversion for FADING.
Adapted from MunchkinChen/FADING (BMVC 2023).

Inverts a real image into Stable Diffusion latent space using DDIM
forward pass + null-text optimization for faithful reconstruction.
"""
...
def load_512(image_path, left=0, right=0, top=0, bottom=0):
    """Load and centre-crop an image to 512x512."""
...
class NullInversion:
    """DDIM inversion + null-text optimization for real image editing."""
    ...
    def get_noise_pred(self, latents, t, is_forward=True, context=None):
        ...
        noise_pred = (noise_pred_uncond +
                      guidance_scale * (noise_prediction_text - noise_pred_uncond))
        if is_forward:
    ...
    def null_optimization(self, latents, num_inner_steps, epsilon):
        ...
        bar = tqdm(total=num_inner_steps * NUM_DDIM_STEPS, desc="Null-text opt")
        for i in range(NUM_DDIM_STEPS):
    ...
    def invert(self, image_path: str, prompt: str, offsets=(0, 0, 0, 0),
               num_inner_steps=5, early_stop_epsilon=1e-5, verbose=False):
        """
        Full null-text inversion pipeline.

        Returns:
            (image_gt, image_rec): Original and VAE-reconstructed images.
            x_t: Final DDIM-inverted latent noise.
            uncond_embeddings: Per-step optimised null-text embeddings.
        """
